...
/* 		//체크박스 작동 메소드
	...
	function init() {
		...
		areaVar = new Vue({
			  ...
			  methods:{					
				  detailview:function(no,jordIn){
					  alert(no);
					  ordDtManagementSelect(no,jordIn);
				  }
			  }  
	...
	function searchOrdDtManagementList(currentPage) {
		...
		var resultCallBackSearch = function(data){
			...
			areaVar.pagenavi = getPaginationHtml(currentPage, totalCnt, ordPageSize, ordPageBlock, 'searchOrdDtManagementList');
			$("#currentPage").val(currentPage);
	...
	//단건 조회
	function ordDtManagementSelect(jordCode,jordIn){
    	if(jordIn == 0)
    	{
    		var param = {
    				jordCode : jordCode
	...
	function frealPopModal(object){
	          ...
	          editVue.jord_CODE = object.jord_CODE;
		      editVue.jord_IN = object.jord_IN;
		      
		      console.log("aa : " +editVue.jord_CODE );
...
<body>
	<form id="myForm" action="" method="">
					...
					<li class="contents">
						...
						<div class="content">
							...
							<select name="searchKey" id="searchKey">
								<option value="cpname" id="option1">발주회사</option>
								<option value="pdname" id="option2">발주제품</option>													
							</select>
						   	...
						   	<input type="checkbox" id="depositCheck" > 승인
								...
								<table class="col">
									...
									<tbody id="listOrdDtManagement" v-for="(item,index) in listitem"
                                                    ...
                                                    <tr @click="detailview(item.jord_CODE,item.jord_IN)">
		...
		<input type="hidden" id="jord_CODE" name="jord_CODE" value="${JORD_CODE}">
		<dl>
			<dt>
