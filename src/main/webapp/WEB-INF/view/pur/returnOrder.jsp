...
<script type="text/javascript">
	...
	function returnOrderList(currentPage) {
		...
		callAjax("/pur/returnOrderList.do", "post", "text", true, param, resultCallBack );
	} 	
	//전체 주문내역 페이징 처리
	function returnOrderListResult(data, currentPage) {
			//console.log("data" + data);

			$("#listreturnOrder").empty().append(data);
			...
			var pagingnavi = getPaginationHtml(currentPage, totalCnt,
					ordPageSize, ordPageBlock, 'returnOrderList');
	...
	function searchreturnOrderList(currentPage) {
			...
			callAjax("/pur/returnOrderList.do", "post", "text", true, param, resultCallBackSearch);
		}

	
		//단건 조회
		function returnOrderone(BORD_CODE) {
			...
			var resultCallBack = function(data) {
				returnOrderResult(data);
			}
			callAjax("/pur/returnOrderone.do", "post", "json", true, param, resultCallBack);
		}
		//단건조회 콜백
		function returnOrderone(data) {
			gfModalPop("#layer1");

			setModal(data.returnOrderModel);
		}
					...
					<tbody>
						<tr>
							...
							<th scope="row">반품회사</th>
								<td><input style="text-align: center;" type="text" class="inputTxt p100" name="" id="" readonly="readonly"/></td><br>
							<th scope="row">제품명</th>
								<td><input style="text-align: center;" type="text" class="inputTxt p100" name="MODEL_NAME" id="MODEL_NAME" readonly="readonly"/></td>
