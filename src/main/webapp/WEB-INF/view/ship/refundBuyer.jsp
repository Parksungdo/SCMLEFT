...
<script type="text/javascript">
	...
	function fReturnList(clickpagenum){
		...
		var searchcallback = function(returndata) {
			...
			searchcallbackprocess(JSON.parse(returndata), clickpagenum);
		...
		function searchcallbackprocess(returndata,clickpagenum) {
			...
			refundList.list = returndata.listrefund;
			
			var totalcnt = $("#refundListCnt").val();
			
			var paginationHtml = getPaginationHtml(clickpagenum, returndata.totalCount, pageSize, pagenumSize, 'fReturnList');
	...
	function fdtlList(re_code, clickpagenum){
		...
		var searchcallback2 = function(returndata) {
			...
			searchcallbackprocess2(JSON.parse(returndata), clickpagenum);
		...
		function searchcallbackprocess2(returndata,clickpagenum) {
			...
			refundDtlList.list = returndata.listrefund;
			var totalcnt = $("#refundDtlListCnt").val();
			
			var paginationHtml = getPaginationHtml(clickpagenum, returndata.totalCount, pageSize, pagenumSize, 'fReturnDtlList');
			console.log("paginationHtml : " + paginationHtml);
			//swal(paginationHtml);
	...
	function refundUpdate(){
		...
		if(confirm('재고처리를 하시겠습니까?')){
			if(refundDtlList.list[0].re_date == null){
				var param = {
						re_code : refundDtlList.list[0].re_code
						,model_code : refundDtlList.list[0].model_code
						,wh_code : refundDtlList.list[0].wh_code
						,re_amt : refundDtlList.list[0].re_amt
						,loginID : refundDtlList.list[0].loginID
					}
					...
					<div class="content">
						...
						<div class="divComGrpCodList">
							<table class="col">
								...
								<tbody id="refundList">
									<tr v-for="(item, index) in list" v-if="list.length" v-on:click="fdtlList(item.re_code)">
										<td>{{ index + 1 }}</td>
										<td>{{ item.company }}</td>
										<td>{{ item.sum }}</td>
										<td>{{ item.re_date }}</td>
