	...
	function init(){
		...
		vueReturn = new Vue({
			...
			data: {
				...
				pagenavi : getPaginationHtml(1, ${result}.returnCount, listCount, pageCount, 'vueReturn.searchReturn'),
				pageNum : 1,
			},
			methods : {
				/* 반품 현황 검색 및 페이징 */
				searchReturn : function(pageNum){
					...
					var listCallBack = function(data) {
						...
						vueReturn.returnList = data.result.returnList;
						vueReturn.pagenavi = getPaginationHtml(currentPage, data.result.returnCount, listCount, pageCount, 'vueReturn.searchReturn');
				...
				showReturnDetail : function(reCode){
					...
					var listCallBack = function(data) {
						...
						returnDetailModalFeel(data.result.returnList[0]);
		...
		vueReturnDetail = new Vue({
			...
			methods: {
				...
				modalClose : function(){
					gfCloseModal();
					vueReturn.searchReturn(vueReturn.pageNum);
				},
			}
		});
	}
...
/* 		vueReturnDetail.reCode = data.result.reCode,
		vueReturnDetail.jordCode = data.result.jordCode,
		vueReturnDetail.pdName = data.result.pdName,
		vueReturnDetail.pdCode = data.result.pdCode,
		vueReturnDetail.reAmt = data.result.reAmt,
		vueReturnDetail.reDate = data.result.reDate,
		vueReturnDetail.reType = data.result.reType,
		vueReturnDetail.total = data.result.total.toLocaleString(); */
	}
</script>
